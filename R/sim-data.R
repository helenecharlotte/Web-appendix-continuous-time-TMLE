sim.data <- function(n, # sample size
    ...
    if (length(form.C)==0) form.C <- function(L0, L.prev, A.prev, A0) -3.95-(K>30 & K<60)*3+(K>75)*1+(K>40)*5-0.4*K^{2/3}-0.24*(K>2 & K<=4)-0.4*(K>4 & K<=9)-(K>9)*0.4*K^{1/5}+0.2*(K>25)*K^{1/4}+0.1*L0+0.2*(A0==1)+0.9*(A0==2)+2.15*L.prev
    ...
    if (length(form.A)==0) form.A <- function(L0, L.prev, A.prev, A0) cbind(-2.1+(1-A0)*1.7+(1-A.prev)*1.8-A.prev*1.7+L.prev*2.1)
    if (length(form.Y)==0) {
    ...
    for (k in 1:K) {
        ...
        if (length(intervention.A)>0){
            A1 <- rmulti(plogis(intervention.A(L0, A0, L.prev, A.prev, A1)))
        }
